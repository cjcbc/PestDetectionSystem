...
import com.gzy.pestdetectionsystem.assembler.detection.PestDetectionAssembler;
...
public class PestDetectionServiceImpl implements PestDetectionService {
    ...
    @Override
    @Transactional
    @CacheEvict(prefix = "detection:record", suffix = "#userId")
    public PestDetectionVO detect(Long userId, PestDetectionDTO dto) {
        ...
        String originalImageName = Paths.get(originalImagePath).getFileName().toString();
        ...
        PestDetectionVO vo = pestDetectionAssembler.toVO(detection);
    ...
    @Override
    @Cache(prefix = "detection:record", suffix = "#userId", ttl = 30, randomTime = 5, timeUnit = TimeUnit.MINUTES)
    public List<PestDetectionVO> getRecords(Long userId) {
        ...
        return pestDetectionAssembler.toVOList(records);
    }
